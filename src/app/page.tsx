...
function followerTotal(social: Record<string, unknown>): number | null {
  const s = social as {
    x?: { followers?: number };
...
function availability(signals: Record<string, unknown>): string | null {
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div>
            ...
            <p className="text-xs text-zinc-500">
              Candidate cockpit · {total} candidates
              {researchingCount > 0 ? (
      ...
      <main className="mx-auto max-w-7xl px-6 py-6">
        ...
        <div className="mb-5 flex flex-wrap gap-2">
          {chips.map((c) => {
            ...
            const href = c.key === "all" ? "/" : `/?status=${c.key}`;
            return (
              <Link
                key={c.key}
                href={href}
